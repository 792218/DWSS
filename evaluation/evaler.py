...
class Evaler(object):
    ...
    def make_kwargs(self, indices, ids, gv_feat, att_feats):
        kwargs = {}
    ...
    def __call__(self, model, rname):
        ...
        results = []
        with torch.no_grad():
            for _, (indices, gv_feat, att_feats) in enumerate(tqdm.tqdm(self.eval_loader, desc=rname)):
                ids = self.eval_ids[indices]
                gv_feat = gv_feat.cuda()
                att_feats = att_feats.cuda()
    
                kwargs = self.make_kwargs(indices, ids, gv_feat, att_feats)
